...
class TextAttributesTxt extends \Tualo\Office\Basic\RouteWrapper
{
    ...
    public static function getTextAttributesTxt($config, $text): array
    {
        $result = [];
        $config_default = [
            'min_word_length' => 1,
    ...
    {
        BasicRoute::add('/wordcount/attributestxt', function ($matches) {
            set_time_limit(300);
            $sql = ' 
            select 
                translations_texts_txt.*,
                translations_meassure_type.* ,
                translations_texts_txt_attributes.id translations_texts_txt_attributes_id
            from 
                translations_texts_txt 
            join translations_meassure_type
            left join translations_texts_txt_attributes
                on (translations_texts_txt.id,translations_texts_txt.type,translations_texts_txt.page,translations_meassure_type.meassure_type)
                = (translations_texts_txt_attributes.id,translations_texts_txt_attributes.type,translations_texts_txt_attributes.page,translations_texts_txt_attributes.meassure_type)
            having translations_texts_txt_attributes_id is null        
            limit 30
            ';
            $list = self::db()->direct($sql);
            foreach ($list as $item) {
                $res = self::getTextAttributesTxt($item, $item['data']);
                $sql = 'replace into translations_texts_txt_attributes
                (
                    meassure_type,
                    id,
                    type,
